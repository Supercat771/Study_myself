...
public class Main_1157 { 
	...
	public static void main(String[] args) {
		...
		String str = sc.next().toUpperCase();   // 입력값을 대문자로
		
		
		int[] count = new int[26];  // 알파벳 26개를 뜻하는 배열 길이 26개 count
		...
		for(int i=0; i<str.length(); i++) {
			...
			int num = str.charAt(i) - 'A' ;  // 각각의 자리의 알파벳을 A를 빼서 그만큼의 차이를 정수로 넣어둠
			
			
			count[num]++; //  count[0]은 A ,count[1]은 B , cout[2]는 C.... 식으로 각각의 알파벳을 인덱스로 표현하면서 
		...
		for(int i=0; i<count.length; i++) {
			...
			if(max < count[i]) {
				...
				max = count[i];  // 최대값은 count[i]가 가장 많을때의 값이니 해당 count[i]를 max 최대값으로 해둠
				
				answer = (char)(i+'A'); // 정답 answer는 그때의 인덱스 i + 문자형 'A'를 합한 아스키코드 알파벳이다.
				
				
				
			}
			
			else if( max == count[i]) { // 빈도수가 같은 다른 알파벳이 있을경우
				
				
				answer ='?';   // -2라고 해도 되는듯 하다
